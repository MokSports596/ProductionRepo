...
export default function Predraft(props){
  ...
  const [onLeagueSetup, setonLeagueSetup] = useState(false)
    ...
    const styles = StyleSheet.create({
        ...
        container: {width: windowWidth, minHeight: 0.1*windowHeight, display: "flex", marginTop: 0.03*windowHeight},
        player: {borderWidth: 1,borderColor: "border: 1px solid rgba(229, 229, 229, 1)", fontSize: 18, padding: 5, paddingLeft: 50, fontWeight: "600"},
        title2: {
            width: windowWidth,
            textAlign: "center",
...
<TouchableOpacity
      ...
      <Text style = {styles.purpleText}>Waiting for manager to begin draft...</Text>
      </View>
    }

    {isLeagueManager && <View>
<TouchableOpacity
        style={styles.button}
      ...
      <TouchableOpacity
        style={styles.buttonNotSelectable}
      >
        <Text style={styles.buttonText}>
          Begin Draft
        </Text>
      </TouchableOpacity>
      <Text style = {styles.purpleText}>You need 2 more members!</Text>
      </View>}
      </View>
    </View>
</View>
...
{onPayout && <View>
    <TouchableOpacity onPress = {() => (setonPayout(false))} style = {{position: "absolute", height: 0.05*windowHeight, width: 0.1*windowWidth, top: 0.08*windowHeight, left: 0.1*windowWidth}}>
    <Image style = {{position: "absolute", height: 0.05*windowHeight, width: 0.1*windowWidth, marginLeft: -0.05*windowWidth}} resizeMode={'cover'} source = {require('../assets/Login/X.png')}/>
    </TouchableOpacity>
<Text style={styles.title2}>Payout</Text>
{isLeagueManager &&      <Image style = {{position: "absolute", height: 0.8*windowHeight, width: 0.9*windowWidth, top: 0.2*windowHeight, alignSelf: "center"}} resizeMode={'cover'} source = {require('../assets/Login/PayoutLeagueMaster.png')}/>
}
{!isLeagueManager &&       <Image style = {{position: "absolute", height: 0.8*windowHeight, width: 0.9*windowWidth, top: 0.2*windowHeight, alignSelf: "center"}} resizeMode={'cover'} source = {require('../assets/Login/PayoutLeaguePlayer.png')}/>
}
</View>
}
{!onPayout && onLeagueSetup && <View>
    <TouchableOpacity onPress = {() => (setonLeagueSetup(false))} style = {{position: "absolute", height: 0.05*windowHeight, width: 0.1*windowWidth, top: 0.08*windowHeight, left: 0.1*windowWidth}}>
      <Image style = {{position: "absolute", height: 0.05*windowHeight, width: 0.1*windowWidth, marginLeft: -0.05*windowWidth}} resizeMode={'cover'} source = {require('../assets/Login/X.png')}/>
</TouchableOpacity>
